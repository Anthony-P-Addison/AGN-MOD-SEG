...
class RemoveChannels(Transform):
    ...
    def __call__(self, img):
        ...
        if self.channels_to_remove is None:
            return img
        channels_to_keep = [
            i for i in range(img.shape[0]) if i not in self.channels_to_remove
        ]
...
class TrainImageMaskDataset(ImageDataset):
    ...
    def __init__(
        ...
        if init_crop_size:
            if not (
                isinstance(init_crop_size, (list, tuple)) and len(init_crop_size) == 3
            ):
                raise ValueError(
                    "init_crop_size must be a list or tuple of 3 integers."
                )
            dict_augmentations.append(
                RandSpatialCropd(
                    keys=["image", "label", "mask"],
                    roi_size=tuple(init_crop_size),
                    random_size=False,
                )
            )
        ...
        dict_augmentations.append(
            RandRotate90d(
                keys=["image", "label", "mask"], prob=dict_aug_prob, spatial_axes=(0, 2)
            )
        )
        ...
        super().__init__(
            image_files, label_files, transform=transform, seg_transform=seg_transform
        )

        if len(mask_files) != len(image_files):
            raise ValueError(
                f"Got {len(mask_files)} masks but {len(image_files)} images"
            )
    ...
    def __getitem__(self, index):
        ...
        if self.initial_mask_transform is not None:
            ...
            if mask.dim() == 3:
                mask = mask.unsqueeze(0)  # Add channel dim if 3D
        ...
        if not (isinstance(img, torch.Tensor) and img.ndim == 4):
            if isinstance(img, torch.Tensor) and img.ndim == 3:
                ...
                raise TypeError(
                    f"Image at index {index} is not 3D or 4D tensor after initial_transform, shape: {img.shape if isinstance(img, torch.Tensor) else type(img)}"
                )
        ...
        if not (isinstance(label, torch.Tensor) and label.ndim == 4):
            if isinstance(label, torch.Tensor) and label.ndim == 3:
                ...
                raise TypeError(
                    f"Label at index {index} is not 3D or 4D tensor after initial_transform, shape: {label.shape if isinstance(label, torch.Tensor) else type(label)}"
                )
        ...
        if not (isinstance(mask, torch.Tensor) and mask.ndim == 4):
            if isinstance(mask, torch.Tensor) and mask.ndim == 3:
                ...
                raise TypeError(
                    f"Mask at index {index} is not 3D or 4D tensor after initial_mask_transform, shape: {mask.shape if isinstance(mask, torch.Tensor) else type(mask)}"
                )
        ...
        try:
            ...
            print(
                f"Error during dictionary augmentation for sample at index {index}: {e}"
            )
            print(
                f"Data shapes before error: img: {img.shape}, label: {label.shape}, mask: {mask.shape}"
            )
            # Include dtypes as well, as they can sometimes be relevant
            print(
                f"Data dtypes before error: img: {img.dtype}, label: {label.dtype}, mask: {mask.dtype}"
            )
            raise RuntimeError(
                f"Dictionary augmentation pipeline failed at index {index}"
            ) from e
...
def create_dataloader(
    ...
    # training and validation split and index
    if val_size == 0:
        raise ValueError(
    ...
    # create training dataset with images, labels, and masks with augmentation transforms
    ...
    # create validation dataset with images, labels, and masks with transforms
    val_ds = ImageDataset(
        val_images,
        val_segs,
...
def get_dataloader(
    ...
    for dataset in datasetlist:
        ...
        val_size = (
            database_config.total_size[dataset] - database_config.train_size[dataset]
        )
