...
def spatial_contrast_aug(
    ...
    if pathology_label.dim() == 3:
        pathology_label = pathology_label.unsqueeze(0)
    if brain_mask.dim() == 3:
        brain_mask = brain_mask.unsqueeze(0)
    if img.dim() != 4:
        raise ValueError(f"Expected 4D image tensor (C,H,W,D), got {img.shape}")
    if pathology_label.dim() != 4 or pathology_label.shape[0] != 1:
        raise ValueError(
            f"Expected 4D pathology label (1,H,W,D), got {pathology_label.shape}"
        )
    if brain_mask.dim() != 4 or brain_mask.shape[0] != 1:
        raise ValueError(f"Expected 4D brain mask (1,H,W,D), got {brain_mask.shape}")
    ...
    if aug_config.uniform_augs:
        ...
        if random.random() < aug_config.uniform_scale_shift:
            brain_factor_multiply = torch.tensor(
                random.uniform(*aug_config.brain_factor_multiply),
                device=img.device,
                dtype=img.dtype,
            )
            brain_factor_intensity = torch.tensor(
                random.uniform(*aug_config.brain_factor_intensity),
                device=img.device,
                dtype=img.dtype,
            )
        ...
        if random.random() < aug_config.prob_lesion_switch:
            possible_tumor_channels = [
                i for i in range(batch_img.shape[0] - 1) if i not in channel_add
            ]
            tumor_channel = random.choice(possible_tumor_channels)
            working_pathology_img = torch.where(
                tumor_mask_bool, batch_img[tumor_channel], img
            )
        ...
        if torch.any(tumor_mask_bool):
            ...
            if random.random() < aug_config.prob_pathology_invert:
                ...
                working_pathology_img = torch.where(
                    tumor_mask_bool, working_pathology_img * -1.0, working_pathology_img
                )
            ...
            if aug_config.prob_tumor_scale_shift:
                ...
                tumor_factor_multiply = torch.tensor(
                    random.uniform(*aug_config.tumor_factor_multiply),
                    device=img.device,
                    dtype=img.dtype,
                )
                tumor_factor_intensity = torch.tensor(
                    random.uniform(*aug_config.tumor_factor_intensity),
                    device=img.device,
                    dtype=img.dtype,
                )
                ...
                working_pathology_img = torch.where(
                    tumor_mask_bool,
                    working_pathology_img * tumor_factor_multiply
            ...
            working_img = torch.where(
                tumor_mask_bool, working_pathology_img, working_img
            )
...
def plot_augmentation(
    ...
    """
    Plot the augmentation parameters and the augmented image, just if you want to check the augemntaitons visually
    """
    # --- Visualization ---
    if torch.any(tumor_mask_bool):
